...
/**
 * Bearer API-key authentication with check/read/write roles and optional policy scoping. Every
...
export class ApiKeyAuth {
  ...
  static require(need) {
    ...
    return async (request) => {
      ...
      if (role !== need && role !== 'readwrite') throw new RateLimitError('FORBIDDEN', `this API key has no ${need} access`);
    };
  }
  ...
  static assertPolicy(key, policy) {
    if (key.policies && !key.policies.includes(policy)) throw new RateLimitError('FORBIDDEN', `this API key has no access to policy "${policy}"`);
  }
